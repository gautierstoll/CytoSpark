...
def distEllipseCluster(clusterA: EllipseCluster, clusterB: EllipseCluster): Double = {
  val minVect = inv(clusterA.ellipseMat + clusterB.ellipseMat) *
    (clusterA.ellipseMat * DenseMatrix(clusterA.mean) + clusterB.ellipseMat * DenseMatrix(clusterB.mean))

  (((minVect - DenseMatrix(clusterA.mean)).t) * clusterA.ellipseMat * (minVect - DenseMatrix(clusterA.mean)) +
    ((minVect - DenseMatrix(clusterB.mean)).t) * clusterB.ellipseMat * (minVect - DenseMatrix(clusterB.mean))).apply(0, 0)
}
